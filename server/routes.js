...
module.exports = (app, passport) => { 
  ...
  app.get('/api/login',(req, res) => { // will render the login page for the user to fill out
    res.render('login.ejs', {message: req.flash('loginMessage')}); // flash message will be '' if nothing has been run through out passport login config function and fail
  })
  app.post('/api/login',passport.authenticate('local-login',{ //this route will be hit when submitting the login form
    //as you might notice we pass 'local-login' to passport.authenticate as first parameter
    //this is telling passport to use the function associated with this name which we set up at the bottom of config.js
    successRedirect: '/', //hits if a successful session in set
    failureRedirect: '/api/login', // hit if we ever pass false or error to done in config.js
